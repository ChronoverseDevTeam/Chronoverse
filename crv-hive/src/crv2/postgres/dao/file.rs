...
pub async fn find_by_path(db: &impl ConnectionTrait, path: &str) -> DaoResult<Option<Model>> {
...
pub async fn upsert(db: &impl ConnectionTrait, path: &str, created_at: i64) -> DaoResult<()> {
    let am = ActiveModel {
...
pub async fn delete(db: &impl ConnectionTrait, path: &str) -> DaoResult<bool> {
