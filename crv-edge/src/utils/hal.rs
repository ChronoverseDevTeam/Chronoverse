...
fn platform_identifiers() -> Vec<String> {
    ...
    for path in ["/etc/machine-id", "/var/lib/dbus/machine-id"] {
        if let Ok(content) = fs::read_to_string(path) {
            let v = content.trim();
            if !v.is_empty() { ids.push(v.to_string()); break; }
        }
    }
    ...
    if let Ok(content) = fs::read_to_string("/sys/class/dmi/id/product_uuid") {
        let v = content.trim();
        if !v.is_empty() { ids.push(v.to_string()); }
    }
        ...
        if output.status.success() {
            ...
            for line in text.lines() {
                if let Some(pos) = line.find("IOPlatformUUID") {
                    let tail = &line[pos..];
                    if let Some(start) = tail.find('"') { if let Some(end) = tail[start+1..].find('"') {
                        let uuid = &tail[start+1..start+1+end];
                        if !uuid.is_empty() { ids.push(uuid.to_string()); }
                    }}
                }
            }
        }
