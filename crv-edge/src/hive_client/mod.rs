...
impl AuthState {
    fn new() -> Self { Self { access_token: None, persist_path: None } }
}
...
impl HiveClient {
    pub async fn connect<D: Into<String>>(dst: D) -> Result<Self, tonic::Status> {
        ...
        Ok(Self { inner, state, heartbeat_handle: None })
    }
    ...
    pub fn set_token<S: Into<String>>(&self, token: S) {
        ...
        if let Some(path) = persist_path {
            ...
            tokio::spawn(async move { let _ = persist_token(&path, &t).await; });
        }
    }
    pub fn clear_token(&self) { self.state.lock().unwrap().access_token = None; }

    pub fn get_token(&self) -> Option<String> { self.state.lock().unwrap().access_token.clone() }
    ...
    pub async fn create_workspace(&mut self, name: String, path: String, device_finger_print: String) -> Result<NilRsp, tonic::Status> {
        let req = CreateWorkspaceReq { name, path, device_finger_print };
        let rsp = self.inner.create_workspace(req).await?;
        self.maybe_update_token_from_metadata(rsp.metadata());
        Ok(rsp.into_inner())
    }
...
impl Clone for HiveClient {
    fn clone(&self) -> Self {
        Self { inner: self.inner.clone(), state: self.state.clone(), heartbeat_handle: None }
    }
}

fn default_jwt_path() -> PathBuf {
    if let Some(home) = dirs::home_dir() { return home.join(".crv").join("jwt"); }
...
impl HiveClient {
    /// 从磁盘读取 JWT 并设置（若存在）
    pub async fn load_token_from_disk<P: Into<PathBuf>>(&self, path: Option<P>) -> Result<bool, io::Error> {
        let path = match path { Some(p) => p.into(), None => default_jwt_path() };
        if let Ok(data) = fs::read_to_string(&path).await {
            let token = data.trim().to_string();
            if !token.is_empty() { self.set_token(token); return Ok(true); }
        }
        Ok(false)
    }
...
mod tests {
    ...
    async fn test_register_user() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _ = client.register(username, "pw".into(), "b@c.d".into()).await?;
        Ok(())
    }
    ...
    async fn test_login_and_create_workspace() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let username = "Alice".to_string();
        let ws = format!("w_{}", ts);
        let _ = client.register(username.clone(), "pw".into(), "b@c.d".into()).await.ok();
        let _ = client.login(username, "pw".into()).await?;
        let _ = client.create_workspace(ws, "C:/tmp/w1".into(), "fp123".into()).await?;
        Ok(())
    }
}
