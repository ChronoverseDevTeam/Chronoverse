...
pub async fn handle(
    ...
    for file in &local_files {
        ...
        if let Some(workspace_path) = path_engine.local_path_to_workspace_path(&local_path) {
            // 如果无法获取文件 meta，则说明文件不存在，跳过此文件
            if state.db.get_file_meta(&workspace_path)?.is_none() {
                continue;
            }

            // 如果文件已经存在于 active file，则跳过此文件
            if state.db.get_active_file_action(&workspace_path)?.is_some() {
                continue;
            }
            ...
            state
                .db
                .set_active_file_action(workspace_path.clone(), Action::Edit)?;
            checkouted_paths.push(workspace_path.to_string());
        }
    }

    Ok(Response::new(CheckoutRsp { checkouted_paths }))
}
