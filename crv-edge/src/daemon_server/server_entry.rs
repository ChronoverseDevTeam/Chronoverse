...
impl CrvEdgeDaemonServerLocalTest {
    ...
    async fn ensure_hive_client(&self) -> Result<(), Status> {
        ...
        // 尝试加载已保存的令牌
        let _ = client
            .load_token_from_disk(None::<std::path::PathBuf>)
            .await;
    ...
    /// 模拟本地文件缓存操作
    fn simulate_local_file_operation(
        &self,
        operation: &str,
        file_path: &str,
        ...
        match operation {
            "checkout" => {
                // 模拟检出文件到工作区
                let checkout_entry =
                    format!("checked_out_{}_{}", self.get_current_timestamp(), file_path);
                Ok(format!(
                    "文件 {} 已检出到工作区: {}",
                    file_path, checkout_entry
                ))
            }
            "submit" => {
                // 模拟提交文件到服务器
                let submit_entry =
                    format!("submitted_{}_{}", self.get_current_timestamp(), file_path);
                Ok(format!(
                    "文件 {} 已提交到服务器: {}",
                    file_path, submit_entry
                ))
            }
            "get_latest" => {
                // 模拟获取最新文件
